...
missing_index <- which(grepl(NA, BEER_AND_BREWERIES))
missing_index
...
max_med_abv_ibu <- merge(max_by_state, median_by_state, by = "State")
max_med_abv_ibu

# merge brewery count with max and med abv and ibu
breweries_heat_map_data <- merge(max_med_abv_ibu, breweries_by_state, by = "State")
colnames(breweries_heat_map_data)[1] = "abb"
breweries_heat_map_data
...
lookup
lookup$abb <- toupper(lookup$abb)
colnames(lookup)[2] = "region"
lookup$region <- tolower(lookup$region)
lookup

# merge breweries and lookup by state name
merged_map_data <- merge(breweries_heat_map_data, lookup, by = "abb")
merged_map_data
...
# merge joined data with states
map.df <- merge(us_states, breweries_heat_map_data, by = "region", all.x = T)
...
library(GGally)
# ggpair wise plot of ABV and IBU by ounces
ggpairs_beers_breweries <- BEER_AND_BREWERIES %>%
  select(Style, ABV, IBU) %>%
  filter(!is.na(ABV) & !is.na(IBU)) %>%
  mutate(ouncesAsFactor = as.factor(Ounces)) %>%
  ggpairs(aes(color = Class))
ggpairs_beers_breweries

# TODO investigate abv by ounces

# filter ale and ipa beers
...
# plots
hist_ale <- ale_abv_ibu %>%
  ggplot(aes(ABV, IBU, color = Ounces)) + geom_point(position = "jitter", size = 2) +
  ggtitle(paste("Histogram of Ale International Bitterness Unit (IBU) v.  Alcohol by Volume (% ABV), n = ", count(ale_abv_ibu))) + ylab("ABV (%)") + xlab("IBU")
hist_ale

hist_ipa <- ipa_abv_ibu %>% 
  ggplot(aes(IBU, ABV, color = Ounces)) + geom_point(position = "jitter", size = 2) + 
  ggtitle(paste("Histogram of IPA International Bitterness Unit (IBU) v.  Alcohol by Volume (% ABV), n = ", count(ale_abv_ibu))) + ylab("ABV (%)") + xlab("IBU")
hist_ipa
...
matrix = confusionMatrix(table(classification, test$Class))
