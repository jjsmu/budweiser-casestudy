...
plot_breweries <- BEER_AND_BREWERIES %>%
  ggplot(aes(ABV, IBU, color = Ounces)) + geom_point(na.rm = TRUE, position = "jitter") +
...
ggplot(data = BEER_AND_BREWERIES) +
  geom_text(aes(IBU, ABV, label = State)) +
...
table
CM <- confusionMatrix(table(classification, testBeers$Class), mode = "everything")
CM
```
## Internal LOOCV
```{r}
...
loocv_table <- table(trainBeers[, c("ABV", "IBU")], trainBeers$Class)
